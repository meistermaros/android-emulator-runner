...
function launchEmulator(apiLevel, target, arch, profile, cores, ramSize, heapSize, sdcardPathOrSize, diskSize, avdName, forceAvdCreation, emulatorBootTimeout, emulatorOptions, disableAnimations, disableSpellChecker, disableLinuxHardwareAcceleration, enableHardwareKeyboard) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield exec.exec(`adb -s emulator-5682 shell input keyevent 82`);
            if (disableAnimations) {
                console.log('Disabling animations.');
                yield exec.exec(`adb -s emulator-5682 shell settings put global window_animation_scale 0.0`);
                yield exec.exec(`adb -s emulator-5682 shell settings put global transition_animation_scale 0.0`);
                yield exec.exec(`adb -s emulator-5682 shell settings put global animator_duration_scale 0.0`);
            }
            if (disableSpellChecker) {
                yield exec.exec(`adb -s emulator-5682 shell settings put secure spell_checker_enabled 0`);
            }
            if (enableHardwareKeyboard) {
                yield exec.exec(`adb -s emulator-5682 shell settings put secure show_ime_with_hard_keyboard 0`);
            }
        }
        finally {
...
function killEmulator() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield exec.exec(`adb -s emulator-5682 emu kill`);
        }
        catch (error) {
...
function waitForDevice(emulatorBootTimeout) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        while (!booted) {
            try {
                let result = '';
                yield exec.exec(`adb -s emulator-5682 shell getprop sys.boot_completed`, [], {
                    listeners: {
                        stdout: (data) => {
