 ...
 */
function launchEmulator(apiLevel, target, arch, profile, cores, ramSize, heapSize, sdcardPathOrSize, diskSize, avdName, forceAvdCreation, emulatorBootTimeout, emulatorOptions, disableAnimations, disableSpellChecker, disableLinuxHardwareAcceleration, enableHardwareKeyboard, emulatorPort) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield waitForDevice(emulatorBootTimeout, emulatorPort);
            yield exec.exec(`adb ${adbDeviceParam} shell input keyevent 82`);
            if (disableAnimations) {
                console.log('Disabling animations.');
                yield exec.exec(`adb ${adbDeviceParam} shell settings put global window_animation_scale 0.0`);
                yield exec.exec(`adb ${adbDeviceParam} shell settings put global transition_animation_scale 0.0`);
                yield exec.exec(`adb ${adbDeviceParam} shell settings put global animator_duration_scale 0.0`);
            }
            if (disableSpellChecker) {
                yield exec.exec(`adb ${adbDeviceParam} shell settings put secure spell_checker_enabled 0`);
            }
            if (enableHardwareKeyboard) {
                yield exec.exec(`adb ${adbDeviceParam} shell settings put secure show_ime_with_hard_keyboard 0`);
            }
        }
        finally {
 ...
 */
function killEmulator(emulatorPort) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const adbDeviceParam = `-s "emulator-${emulatorPort}"`;
            console.log(`::group::Terminate Emulator`);
            yield exec.exec(`adb ${adbDeviceParam} emu kill`);
        }
        catch (error) {
 ...
 */
function waitForDevice(emulatorBootTimeout, emulatorPort) {
    return __awaiter(this, void 0, void 0, function* () {
        let booted = false;
        let attempts = 0;
        ...
        while (!booted) {
            try {
                let result = '';
                yield exec.exec(`adb ${adbDeviceParam} shell getprop sys.boot_completed`, [], {
                    listeners: {
                        stdout: (data) => {
